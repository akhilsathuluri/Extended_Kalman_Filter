function J = JacobianSystem(y, u)
        ...
        f(1) = (F_sc*(C_sc-C_rc)/W) - k*O_d*C_rc;
        f(2) = (Ra*(O_in - O_d)/Wa) - (((n+2+((n+4)*sig))*k*O_d*C_rc*W)/(4*M_c*Wa*(1+sig)));
        f(3) = (T_ri1*F_sc/W) + (T_a*F_a*c_pa/(W*c_ps)) - (T_rg*(F_sc*c_ps + F_a*c_pa)/(W*c_ps)) - (delH*k*O_d*C_rc/(c_ps*M_c));
        
        tempJ = jacobian([f(1) f(2) f(3)],[C_rc O_d T_rg]);
